...
const ToothIconBg = () => (
  <Box sx={{ 
    ...
    boxShadow: '0 8px 20px rgba(0, 180, 216, 0.3)', mb: 3, color: '#ffffff' 
  }}>
...
const ALL_TIME_SLOTS = ['NINE', 'NINE_15', 'NINE_30', 'NINE_45', 'TEN', 'TEN_15', 'TEN_30', 'TEN_45', 'ELEVEN', 'ELEVEN_15', 'ELEVEN_30', 'ELEVEN_45', 'TWELVE', 'TWELVE_15', 'TWELVE_30', 'TWELVE_45', 'FOURTEEN', 'FOURTEEN_15', 'FOURTEEN_30', 'FOURTEEN_45', 'FIFTEEN', 'FIFTEEN_15', 'FIFTEEN_30', 'FIFTEEN_45', 'SIXTEEN', 'SIXTEEN_15', 'SIXTEEN_30', 'SIXTEEN_45', 'SEVENTEEN'];
...
const BookAppointment = () => {
  ...
  const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', phone: '', date: '', duration: 15, selectedSlot: '', notes: '', reason: '' });
  ...
  // PRECOMPILAZIONE CAMPI (Incluso Telefono)
  useEffect(() => {
    if (user) {
      setFormData(prev => ({ 
        ...prev, 
        ...
        email: user.email || '', 
        // Cerchiamo il telefono in varie proprietà comuni
    ...
    if (bookingSuccess) {
      if (isProcessingPayPal && !paypalOrderId && !paymentLoading && !paypalRequestSent.current) {
        ...
        if (lastAppt?.id) {
          ...
          dispatch(createPayPalPaymentRequest({ amount: calculatePrice(formData.duration).toFixed(2), appointmentId: lastAppt.id }));
        }
  ...
  const handleSubmit = () => {
    ...
    const data = { 
        doctorId: parseInt(selectedDoctor.id, 10), patientId: user?.id, appointmentDate: formData.date, 
        timeSlot: formData.selectedSlot, durationMinutes: parseInt(formData.duration, 10), status: 'BOOKED', 
        notes: formData.notes || '', reason: formData.reason || '', phone: formData.phone || '' 
    };
      ...
      <Container maxWidth="lg">
        <Paper elevation={0} sx={{ p: { xs: 4, md: 6 }, borderRadius: 5, bgcolor: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 40px rgba(0, 180, 216, 0.15)' }}>
          ...
          <Box sx={{ textAlign: 'center', mb: 5 }}>
            <ToothIconBg />
            <Typography variant="h3" sx={{ fontWeight: 800, color: '#00B4D8', letterSpacing: '-0.5px' }}>Prenota la tua Visita</Typography>
            <Typography variant="body1" color="text.secondary" sx={{ mt: 1 }}>Completa i passaggi per fissare un appuntamento</Typography>
          </Box>

          {(error || paymentError) && <Alert severity="error" sx={{ mb: 4, borderRadius: 3 }} onClose={() => dispatch(resetPaymentState())}>{error || paymentError}</Alert>}
                ...
                <Grid item xs={12} md={5}>
                  <Card sx={{ p: 4, border: '2px solid #E0F7FA', bgcolor: '#F8FDFF', borderRadius: 4, height: '100%', boxShadow: 'none' }}>
                    <Stack direction="row" alignItems="center" spacing={2} mb={3}>
                        <CalendarMonthIcon sx={{ color: '#00B4D8', fontSize: 32 }} />
                        <Typography variant="h5" sx={{ fontWeight: 700, color: '#0077B6' }}>Quando?</Typography>
                    </Stack>
                    ...
                    <FormControl fullWidth variant="outlined">
                      ...
                      <Select value={formData.duration} label="Durata della Visita" onChange={(e) => setFormData({...formData, duration: e.target.value})}>
                        <MenuItem value={15}>15 min - €35</MenuItem>
                        <MenuItem value={30}>30 min - €60</MenuItem>
                        <MenuItem value={60}>1 ora - €100</MenuItem>
                      </Select>
                    </FormControl>
                  </Card>
                </Grid>
                          ...
                          {ALL_TIME_SLOTS.map((slot) => {
                            ...
                            return (
                              <Tooltip key={slot} title={isAvailable ? "Clicca per selezionare" : "Non disponibile"} arrow placement="top">
                                <Chip label={formatSlotTime(slot)} onClick={() => isAvailable && setFormData({ ...formData, selectedSlot: slot })} icon={isAvailable ? <AccessTimeIcon /> : <BlockIcon />} sx={{ cursor: isAvailable ? 'pointer' : 'not-allowed', bgcolor: isSelected ? '#00B4D8' : (isAvailable ? '#ffffff' : '#f5f5f5'), color: isSelected ? '#ffffff' : (isAvailable ? '#00B4D8' : '#9e9e9e'), border: `2px solid ${isSelected ? '#00B4D8' : (isAvailable ? '#CAF0F8' : 'transparent')}`, fontWeight: isSelected ? 700 : 600, height: 42, px: 1, boxShadow: isSelected ? '0 4px 12px rgba(0, 180, 216, 0.4)' : 'none', transform: isSelected ? 'scale(1.05)' : 'scale(1)', transition: 'all 0.2s ease-in-out' }} />
                              </Tooltip>
                            );
                          })}
                        </Box>
            ...
            {activeStep === 1 && (
              <Box>
                 ...
                 <Grid container spacing={3} sx={{ mb: 5 }}>
                  ...
                  <Grid item xs={12} md={6}><TextField fullWidth label="Telefono" name="phone" value={formData.phone} onChange={handleInputChange} InputProps={{ startAdornment: <InputAdornment position="start"><PhoneIcon color="primary"/></InputAdornment> }} variant="filled" sx={{borderRadius: 2, overflow: 'hidden'}}/></Grid>
                </Grid>
                  ...
                  <Grid container spacing={4} justifyContent="center">
                      <Grid item xs={12} md={5}>
                          <Card onClick={() => dispatch(setPaymentMethod('PAYPAL'))} sx={{ p: 3, cursor: 'pointer', transition: 'all 0.3s', border: `3px solid ${paymentMethod === 'PAYPAL' ? '#0070BA' : 'transparent'}`, bgcolor: paymentMethod === 'PAYPAL' ? '#F0F8FF' : '#fff', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 8px 25px rgba(0,112,186,0.2)' }, borderRadius: 4 }}>
                              ...
                              <Divider sx={{my: 2}}/>
                              <Typography variant="h4" color="#0070BA" fontWeight={800} align="right">€{appointmentPrice.toFixed(2)}</Typography>
                          </Card>
                      </Grid>
                      <Grid item xs={12} md={5}>
                          <Card onClick={() => dispatch(setPaymentMethod('CASH'))} sx={{ p: 3, cursor: 'pointer', transition: 'all 0.3s', border: `3px solid ${paymentMethod === 'CASH' ? '#4CAF50' : 'transparent'}`, bgcolor: paymentMethod === 'CASH' ? '#F1F8F4' : '#fff', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', '&:hover': { transform: 'translateY(-5px)', boxShadow: '0 8px 25px rgba(76,175,80,0.2)' }, borderRadius: 4 }}>
                              ...
                              <Divider sx={{my: 2}}/>
                              <Typography variant="h4" color="#4CAF50" fontWeight={800} align="right">€{appointmentPrice.toFixed(2)}</Typography>
                          </Card>
                      </Grid>
                  </Grid>
               </Box>
              ...
              <Box sx={{ maxWidth: 800, mx: 'auto' }}>
                <Card sx={{ p: 5, border: 'none', background: 'linear-gradient(145deg, #E1F5FE, #F0F9FF)', borderRadius: 5, mb: 5, boxShadow: 'inset 0 0 30px rgba(0,180,216,0.05)' }}>
                  ...
                  <Grid container spacing={4}>
                      ...
                      <Grid item xs={12} sm={6}><Typography variant="subtitle2" color="textSecondary">Medico</Typography><Typography variant="h6" fontWeight={600}>Dr. {selectedDoctor.name} {selectedDoctor.surname}</Typography></Grid>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant="h6">Importo Totale</Typography>
                      <Typography variant="h3" sx={{ fontWeight: 800, color: '#00B4D8' }}>€{appointmentPrice.toFixed(2)}</Typography>
                  </Box>
                </Card>
